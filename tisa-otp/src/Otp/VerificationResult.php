...
final class VerificationResult {
	...
	public function message(): string {
		switch ( $this->status ) {
			case self::ACCEPTED:
				return __( 'کد تأیید پذیرفته شد.', 'tisa-otp' );

			case self::EXPIRED:
			case self::MISSING:
				return __( 'کد تأیید منقضی شده است. لطفاً کد تازه درخواست کنید.', 'tisa-otp' );

			case self::EXHAUSTED:
				return __( 'تلاش‌های مجاز برای این کد تمام شد. لطفاً کد تازه بگیرید.', 'tisa-otp' );

			case self::MALFORMED:
				return __( 'قالب کد واردشده درست نیست.', 'tisa-otp' );

			case self::MISMATCH:
			default:
				return sprintf(
					/* translators: %d: remaining attempts */
					__( 'کد واردشده درست نیست. %d تلاش باقی مانده است.', 'tisa-otp' ),
					$this->attemptsLeft
				);
		}
	}

...
final class Logger {
	...
	private function record( string $severity, string $event, array $context ): void {
		 ...
		 */
		do_action( 'tisa_otp_log', $severity, $event, $context );
		...
		if ( $this->settings->bool( 'debug', false ) && defined( 'WP_DEBUG' ) && WP_DEBUG ) {
			error_log( sprintf( '[tisa-otp][%s] %s %s', $severity, $event, wp_json_encode( $safe ) ) ); // phpcs:ignore WordPress.PHP.DevelopmentFunctions.error_log_error_log
		}
	}
}

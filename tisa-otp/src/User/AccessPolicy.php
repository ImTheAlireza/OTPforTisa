...
final class AccessPolicy {
	...
	public function guardedRoles(): array {
		 ...
		 */
		return array_values( array_unique( (array) apply_filters( 'tisa_otp_guarded_roles', $roles ) ) );
	}
	...
	public function denialMessage(): string {
		return __( 'ورود با کد یکبارمصرف برای این نقش کاربری غیرفعال است. لطفاً از رمز عبور استفاده کنید.', 'tisa-otp' );
	}
	...
	public static function selectableRoles(): array {
		 ...
		 */
		return (array) apply_filters( 'tisa_otp_selectable_roles', $roles );
	}
	...
	private function filter( bool $allowed, \WP_User $user ): bool {
		 ...
		 */
		return (bool) apply_filters( 'tisa_otp_allows_user', $allowed, $user );
	}
}

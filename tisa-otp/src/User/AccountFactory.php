...
final class AccountFactory {
	...
	public function create( string $phone, array $values = array(), array $fields = array() ) {
		...
		if ( ! Phone::isValid( $phone ) ) {
			return new \WP_Error( 'invalid_phone', __( 'شماره موبایل معتبر نیست.', 'tisa-otp' ) );
		}
		...
		if ( null === $result ) {
			return new \WP_Error( 'busy', __( 'درخواست دیگری برای این شماره در حال پردازش است. چند لحظه صبر کنید.', 'tisa-otp' ) );
		}
	...
	private function build( string $phone, array $values, array $fields ) {
		...
		if ( '' !== $email && email_exists( $email ) ) {
			return new \WP_Error( 'email_taken', __( 'این ایمیل پیش‌تر ثبت شده است. ایمیل دیگری وارد کنید یا با همان حساب وارد شوید.', 'tisa-otp' ) );
		}
		 ...
		 */
		$args = (array) apply_filters( 'tisa_otp_new_user_args', $args, $phone, $values );
		...
		update_user_meta( $userId, 'tisa_signup_channel', 'otp' );
		 ...
		 * @param array  $values Sanitised field values.
		 */
		do_action( 'tisa_otp_user_created', $userId, $phone, $values );
	...
	private function username( string $phone, string $email, array $values ) {
		 ...
		 */
		$base = sanitize_user( (string) apply_filters( 'tisa_otp_username_base', $base, $phone, $values ), true );
		...
		while ( username_exists( $username ) ) {
			...
			if ( $suffix > 500 ) {
				return new \WP_Error( 'username_exhausted', __( 'امکان ساخت نام کاربری یکتا وجود ندارد.', 'tisa-otp' ) );
			}
		}
	...
	public function role(): string {
		 ...
		 */
		return sanitize_key( (string) apply_filters( 'tisa_otp_default_role', $role ) );
	}

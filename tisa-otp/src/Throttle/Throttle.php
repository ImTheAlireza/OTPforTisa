...
namespace TisaOtp\Throttle;

use TisaOtp\Config\Settings;
use TisaOtp\State\StateStore;
use TisaOtp\Support\ClientIp;
...
final class Throttle {
	...
	public function chargeSend( string $phone, string $ip ): void {
		...
		if ( $perPhone > $phoneLimit ) {
			throw Rejection::make(
				'quota_phone',
				__( 'سقف ارسال کد برای این شماره در بازه جاری پر شده است. کمی بعد دوباره تلاش کنید.', 'tisa-otp' ),
		...
		if ( $perIp > $ipLimit ) {
			throw Rejection::make(
				'quota_ip',
				__( 'تعداد درخواست‌ها از آدرس شما بیش از حد مجاز است.', 'tisa-otp' ),
				array( 'retry_after' => $window )
			);
		}
		...
		if ( $perDay > $dayLimit ) {
			throw Rejection::make(
				'quota_ip_daily',
				__( 'سقف روزانه ارسال کد از این آدرس تکمیل شده است.', 'tisa-otp' ),
	...
	public function chargeVerify( string $ip ): void {
		...
		if ( $hits > $limit ) {
			throw Rejection::make( 'quota_verify', __( 'تلاش‌های تأیید کد از این آدرس بیش از حد مجاز است.', 'tisa-otp' ), array( 'retry_after' => $window ) );
		}
	}
	...
	public function chargeChallenge( string $ip ): void {
		...
		if ( $hits > $limit ) {
			throw Rejection::make( 'quota_challenge', __( 'تعداد درخواست‌های امنیتی از این آدرس بیش از حد مجاز است.', 'tisa-otp' ) );
		}
	}

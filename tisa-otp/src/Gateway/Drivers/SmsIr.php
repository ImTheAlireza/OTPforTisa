...
final class SmsIr extends HttpGateway implements AccountProbe {
	...
	public function fields(): array {
		return array(
			...
			'smsir_template_id' => array(
				...
				'type'  => 'text',
				'hint'  => __( 'خالی بماند، پیامک متنی معمولی فرستاده می‌شود.', 'tisa-otp' ),
			),
			'smsir_sender'      => array(
				'label' => __( 'شماره خط', 'tisa-otp' ),
				'type'  => 'text',
				'hint'  => __( 'فقط برای پیامک متنی لازم است.', 'tisa-otp' ),
			),
		);
	}
	...
	public function plan(): array {
		...
		if ( '' !== trim( $this->option( 'smsir_template_id' ) ) && '' === $template ) {
			$issues[] = __( 'شناسه الگو باید عدد باشد؛ همان عددی که در بخش «ارسال سریع» پنل SMS.ir آمده است.', 'tisa-otp' );
		}

		if ( '' !== trim( $this->option( 'smsir_sender' ) ) && '' === $sender ) {
			$issues[] = __( 'شماره خط باید عدد باشد؛ همان شماره‌ای که در پنل SMS.ir برای شما فعال است.', 'tisa-otp' );
		}

		if ( '' === $template && '' === $sender ) {
			$issues[] = __( 'نه شناسه الگو و نه شماره خط تنظیم شده است؛ SMS.ir بدون هیچ‌کدام پیامکی نمی‌پذیرد.', 'tisa-otp' );
		}
	...
	private function sendVerify( DeliveryRequest $request, array $args ): GatewayResult {
		 ...
		 */
		$name = trim( (string) apply_filters( 'tisa_otp_smsir_param', 'CODE' ) );
		$code = $request->code();

		if ( '' === $name ) {
			return $this->reject( __( 'نام پارامتر الگو خالی است؛ مقدار پیش‌فرض CODE را نگه دارید.', 'tisa-otp' ), 'SMS.ir: empty parameter name' );
		}
		...
		if ( mb_strlen( $code ) > self::PARAM_MAX ) {
			return $this->reject(
				sprintf(
					...
					__( 'کد تولیدشده بیش از %d نویسه است؛ SMS.ir آن را در الگو نمی‌پذیرد.', 'tisa-otp' ),
					self::PARAM_MAX
				),
	...
	private function sendBulk( DeliveryRequest $request, array $args ): GatewayResult {
		...
		if ( '' === $sender ) {
			return $this->notConfigured( __( 'برای ارسال متنی SMS.ir شماره خط لازم است؛ در تنظیمات آن را وارد کنید.', 'tisa-otp' ) );
		}
	...
	private function issue( ?int $api, int $http, array $body ): array {
		...
		if ( null !== $api ) {
			return array(
				...
				'message' => '' !== $sentence ? $sentence : __( 'سامانهٔ SMS.ir درخواست را نپذیرفت.', 'tisa-otp' ),
				'meta'    => array(
					'api_status' => $api,
		...
		return array(
			'code'    => $this->codeForStatus( $http ),
			'message' => '' !== $sentence ? $sentence : __( 'پاسخ SMS.ir قابل خواندن نبود.', 'tisa-otp' ),
			'meta'    => array(
	...
	public function probe(): array {
		...
		if ( '' === $apiKey ) {
			...
			$out['message']    = __( 'کلید API سامانه SMS.ir تنظیم نشده است.', 'tisa-otp' );
		...
		if ( $lines['ok'] && is_array( $lines['data'] ) ) {
			...
			if ( '' !== $out['sender'] ) {
				...
				if ( ! $out['sender_ok'] ) {
					...
					$out['message']       = __( 'شماره خطی که در تنظیمات گذاشته‌اید در فهرست خطوط این حساب نیست؛ شماره را از پنل SMS.ir کپی کنید.', 'tisa-otp' );

...
final class Ippanel extends HttpGateway {
	...
	public function fields(): array {
		return array(
			...
			'ippanel_pattern' => array(
				...
				'type'  => 'text',
				'hint'  => __( 'اگر خالی بماند، پیام متنی معمولی ارسال می‌شود.', 'tisa-otp' ),
			),
			'ippanel_sender'  => array(
				'label' => __( 'شماره فرستنده', 'tisa-otp' ),
				'type'  => 'text',
				'hint'  => __( 'برای هر دو حالت لازم است؛ نمونه: +983000505', 'tisa-otp' ),
			),
		);
	}
	...
	public function plan(): array {
		...
		if ( '' === trim( $this->option( 'ippanel_api_key' ) ) ) {
			$issues[] = __( 'کلید API آی‌پی‌پنل تنظیم نشده است.', 'tisa-otp' );
		}

		if ( '' === $sender ) {
			$issues[] = __( 'شماره فرستنده خالی است؛ API آی‌پی‌پنل بدون شماره خط پیام را نمی‌پذیرد.', 'tisa-otp' );
		}
		...
		return array(
			...
			'issues'   => $issues,
			'notes'    => '' !== $pattern
				? array( __( 'متغیر پترن باید «code» باشد؛ در غیر این صورت نام متغیر را با فیلتر tisa_otp_ippanel_param عوض کنید.', 'tisa-otp' ) )
				: array(),
		);
	}
	...
	public function deliver( DeliveryRequest $request ): GatewayResult {
		...
		if ( '' === $sender ) {
			return $this->notConfigured( __( 'برای آی‌پی‌پنل شماره فرستنده را در تنظیمات وارد کنید.', 'tisa-otp' ) );
		}
		...
		if ( '' !== $pattern ) {
			 ...
			 */
			$param = (string) apply_filters( 'tisa_otp_ippanel_param', 'code' );

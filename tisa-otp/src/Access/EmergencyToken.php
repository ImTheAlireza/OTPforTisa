...
final class EmergencyToken {
	...
	const OPTION       = 'tisa_otp_emergency';
	const MIN_LENGTH   = 6;
	const MAX_LENGTH   = 12;
	...
	public function issue( string $code, int $minutes, int $uses, bool $ipLock, array $phones, string $ip ): array {
		 ...
		 */
		do_action( 'tisa_otp_emergency_issued', (int) $state['expires_at'], $uses, count( $phones ) );
	...
	public function revoke(): void {
		 ...
		 */
		do_action( 'tisa_otp_emergency_revoked' );
	}
	...
	public function hold( string $code, int $userId ): void {
		if ( $userId > 0 ) {
			set_transient( 'tisa_otp_emergency_reveal_' . $userId, $code, self::HOLD_SECONDS );
		}
	}

	public function pull( int $userId ): string {
		$key  = 'tisa_otp_emergency_reveal_' . $userId;

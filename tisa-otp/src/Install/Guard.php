...
final class Guard implements Bootable {
	...
	private function printFailures(): void {
		...
		echo '<div class="notice notice-error"><p><strong>'
			. esc_html__( 'تیسا OTP نتوانست کامل بالا بیاید.', 'tisa-otp' )
			. '</strong> ';

		echo esc_html__( 'این بخش‌ها شروع نشدند، پس تا وقتی این پیام هست بخشی از افزونه کار نمی‌کند:', 'tisa-otp' );
		echo ' <code>' . esc_html( implode( '</code> <code>', array_unique( $ids ) ) ) . '</code></p>';

		echo '<p>' . esc_html__( 'نشانهٔ شناخته‌شدهٔ این خطا: فایل‌های افزونه از دو نسخهٔ مختلف‌اند (بستهٔ نصب‌شده کامل جایگزین نشده). بستهٔ کامل همان نسخه را از نو نصب کنید: افزونه‌ها ← افزودن ← بارگذاری افزونه ← «جایگزینی با نسخهٔ بارگذاری‌شده».', 'tisa-otp' ) . '</p>';
	...
	private function printMismatch( array $state ): void {
		...
		echo '<div class="notice notice-warning"><p><strong>'
			. esc_html__( 'تیسا OTP: فایل‌های افزونه با هم نمی‌خوانند.', 'tisa-otp' )
			. '</strong> ';

		if ( 'manifest_missing' === $state['note'] ) {
			echo esc_html__( 'فهرست بستهٔ افزونه (build.json) پیدا نشد، پس معلوم نیست کدام نسخه کامل نصب شده است.', 'tisa-otp' );
		} else {
			printf(
				/* translators: 1: number of files, 2: plugin version */
				esc_html__( '%1$d فایل با نسخهٔ %2$s نمی‌خواند. اگر همین حالا بسته را به‌روز کرده‌اید، یعنی همهٔ فایل‌ها جایگزین نشده‌اند.', 'tisa-otp' ),
				count( $offenders ),
		...
		if ( ! empty( $offenders ) ) {
			...
			if ( count( $offenders ) > count( $shown ) ) {
				printf(
					...
					esc_html__( ' و %d فایل دیگر', 'tisa-otp' ),
		...
		echo '<p>' . esc_html__( 'راه‌حل: بستهٔ کامل نسخهٔ همین افزونه را از نو نصب کنید — افزونه‌ها ← افزودن ← بارگذاری افزونه ← «جایگزینی با نسخهٔ بارگذاری‌شده». با جایگزینی فایل‌ها هیچ تنظیمات یا داده‌ای پاک نمی‌شود؛ هرگز افزونه را «حذف» نکنید.', 'tisa-otp' ) . '</p>';
		echo '</div>';
	}
}

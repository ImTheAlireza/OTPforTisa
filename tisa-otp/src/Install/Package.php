...
final class Package {
	...
	const CACHE    = 'tisa_otp_package_state';
	...
	public static function manifest( string $root = '' ): ?array {
		$path = ( '' === $root ? TISA_OTP_PATH : trailingslashit( $root ) ) . self::MANIFEST;
	...
	public static function verify( string $root = '', bool $fresh = false ): array {
		$root   = '' === $root ? TISA_OTP_PATH : trailingslashit( $root );
		$cached = ( '' === $root || $root === TISA_OTP_PATH ) ? self::$state : null;
		...
		$state = array(
			'ok'      => false,
			'version' => defined( 'TISA_OTP_VERSION' ) ? TISA_OTP_VERSION : '',
			'checked' => 0,
			'stale'   => array(),
			'missing' => array(),
			'note'    => '',
	...
	public static function cached(): ?array {
		...
		$manifest = TISA_OTP_PATH . self::MANIFEST;

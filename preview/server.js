	...
	const rows = {
		...
		gateways: {
			...
			rows: [
				...
				{ label: 'کلید API و اعتبار', value: 'پذیرفته شد · اعتبار ۱۶۵٫۳', status: 'ok', note: 'SMS.ir: key accepted, credit 165.3' },
				{ label: 'شماره خط', value: '3000505', status: 'warn', note: 'خط‌های این حساب: 10002155613464 — شماره خطی که در تنظیمات گذاشته‌اید در این فهرست نیست؛ شماره را از پنل SMS.ir کپی کنید.' },
		...
		security: {
			...
			rows: [
				...
				{ label: 'ردشدن به‌خاطر کپچا (۷ روز)', value: '۹ درخواست', status: 'fail', note: '۹ درخواست بدون توکن کپچا رسیده است؛ ویجت در مرورگر کاربران بارگذاری نشده. جایگزین: نشانی اسکریپت یا خاموش کردن کپچا.' },
...
function handleRest(route, body, headers) {
	...
	switch (route) {
		...
		case 'admin/test':
			...
			/*
			 * The case the owner photographed: the panel is unreachable because
			 * the site blocks outbound HTTP, the email channel still delivers
			 * the code, and the modal has to say exactly that instead of a
			 * green «ارسال شد» over a failed SMS test.
			 */
			return ok({
				sent: true,
				via: 'smsir',

...
| `../tools/css-check.js` | استایل‌شیت‌ها با پارسر واقعی خوانده می‌شوند: خطای نگارشی، انتخاب‌گری بیرون از فضای نام افزونه (`button {}`)، و هر `url()` که به فایل موجود اشاره نمی‌کند. این دروازه از یک خرابی واقعی آمده: یک `@font-face` ناقص می‌تواند بقیهٔ فایل را بی‌صدا ببلعد. | Node + css-tree |
...
| `php/hardening-test.php` | چیزهایی که مهاجم اول به آن‌ها می‌رسد: یک کد درست فقط یک‌بار خرج می‌شود (حتی وقتی دو درخواست در همان ثانیه با هم برسند — با `claim` اتمی روی جدول)، ورود با کد رویداد `wp_login` را صدا می‌زند تا بقیهٔ وردپرس هم آن را ببیند، سقف روزانهٔ کل سایت جلوی حمله از هزاران آدرس را می‌گیرد، درخواستی که هیچ‌کدام از دو نشانهٔ فرم را ندارد رد می‌شود، و خروجی CSV نمی‌تواند فرمول اکسل را به سیستم مدیر ببرد. | PHP 7.4+ |
| `php/woodmart-test.php` | پنل ورود وودمارت: افزونه همان هوکی را که پوسته فرم را با آن چاپ می‌کند قاب می‌گیرد (بافر یک اولویت قبل باز، یکی بعد بسته) و فقط فرم ووکامرس داخل همان پنل را با فرم OTP عوض می‌کند. اولویت از خود رجیستری وردپرس خوانده می‌شود، پس اگر آپدیت وودمارت جای فراخوانی را عوض کند، قاب هم با آن جابه‌جا می‌شود. خروجی غیرِ پنل بایت‌به‌بایت برمی‌گردد، حالت «جایگزین/در کنار» هر دو آزمایش می‌شوند، و یک شرط ساختاری نشان می‌دهد هیچ فرمی داخل فرم دیگر نمی‌رود. | PHP 7.4+ |
